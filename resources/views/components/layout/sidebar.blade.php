@php
    ...
    if ($role === 'admin') {
        $navigation = [
            'System Overview' => [
                ['label' => 'Overview', 'icon' => 'presentation-chart-line', 'url' => route('admin.dashboard'), 'active' => request()->routeIs('admin.dashboard')],
                ['label' => 'Live Activity', 'icon' => 'signal', 'url' => '#'],
            ],
            'User Management' => [
                ['label' => 'Users & Roles', 'icon' => 'users', 'url' => '#'],
                ['label' => 'Permissions', 'icon' => 'key', 'url' => '#'],
            ],
            'Content Oversight' => [
                ['label' => 'Courses Review', 'icon' => 'book-open', 'url' => '#'],
                ['label' => 'Flagged Content', 'icon' => 'flag', 'url' => '#'],
            ],
            'Learning Activity' => [
                ['label' => 'Attendance', 'icon' => 'clipboard-document-list', 'url' => '#'],
                ['label' => 'Learner Progress', 'icon' => 'chart-bar', 'url' => '#'],
            ],
            'Academic Performance' => [
                ['label' => 'Quizzes & Assignments', 'icon' => 'academic-cap', 'url' => '#'],
                ['label' => 'Portfolios', 'icon' => 'folder-open', 'url' => '#'],
            ],
            'Communication' => [
                ['label' => 'Chat Moderation', 'icon' => 'chat-bubble-left-right', 'url' => '#'],
                ['label' => 'AI Monitoring', 'icon' => 'cpu-chip', 'url' => '#'],
            ],
            'System Events' => [
                ['label' => 'Notifications', 'icon' => 'bell', 'url' => '#'],
                ['label' => 'System Logs', 'icon' => 'list-bullet', 'url' => '#'],
            ],
            'Security' => [
                ['label' => 'Audit & Security', 'icon' => 'shield-check', 'url' => '#'],
                ['label' => 'Access Logs', 'icon' => 'lock-closed', 'url' => '#'],
            ],
            'Configuration' => [
                ['label' => 'Settings', 'icon' => 'cog-6-tooth', 'url' => '#'],
                ['label' => 'Integrations', 'icon' => 'puzzle-piece', 'url' => '#'],
            ],
        ];
        ...
        <nav class="space-y-4">
            ...
            @foreach($navigation as $key => $item)
                @if(is_array($item) && is_string($key))
                    {{-- Section Group --}}
                    @php
                        $hasActiveChild = collect($item)->contains(fn($link) => $link['active'] ?? false);
                    @endphp
                    <div x-data="{ open: {{ $hasActiveChild ? 'true' : 'false' }} }">
                        <button @click="open = !open" class="flex items-center justify-between w-full px-2 py-2 mt-4 text-xs font-semibold text-secondary uppercase tracking-wider hover:text-primary transition-colors group">
                            <span>{{ $key }}</span>
                            <svg class="w-3 h-3 text-secondary/50 transition-transform duration-200"
                                 :class="{ 'rotate-180': open, 'text-primary': open }"
                        ...
                        <div x-show="open" 
                            ...
                            <a href="{{ $link['url'] }}"
                               class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors duration-200 group
                                      {{ ($link['active'] ?? false) ? 'bg-[var(--accent-primary)]/10 text-accent' : 'hover:surface-muted text-secondary hover:text-accent' }}">
                                @if(isset($link['icon']))
                                    @svg('heroicon-o-'.$link['icon'], 'w-5 h-5 group-hover:text-accent transition-colors ' . (($link['active'] ?? false) ? 'text-accent' : 'text-secondary'))
                                @endif
                ...
                @else
                    ...
                    <a href="{{ $item['url'] }}"
                       class="flex items-center gap-3 px-3 py-2.5 rounded-lg transition-colors duration-200 hover:surface-muted text-secondary hover:text-accent group">
                        @if(isset($item['icon']))
                            @svg('heroicon-o-'.$item['icon'], 'w-5 h-5 text-secondary group-hover:text-accent transition-colors')
                        @endif
